...
public class Scraper {
    ...
    public Scraper(String url, String target) throws IOException {
        this.url = url;
    ...
    public void download(Boolean blur) {
        for (Element link : images) {
            BufferedImage image;
            try{
                URL url =new URL(this.url + link.attr("href"));
                image = ImageIO.read(url);
                if (!blur) ImageIO.write(image, "png",new File(this.target+link.attr("href")));
                else {
                    int[] filter = new int[] {1, 2, 1, 2, 4, 2, 1, 2, 1};
                    BufferedImage imgBlured = blur(image, filter, 3);
                    ImageIO.write(imgBlured, "png", new File(this.target+link.attr("href")));
                }
            } catch(IOException e){
                e.printStackTrace();
            }
    ...
    public static void main(String[] args) throws IOException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        PrintWriter pw = new PrintWriter(new FileOutputStream("times_seq.dat"));
        pw.println(time.toString());
        pw.close();
        PrintWriter pw_blur = new PrintWriter(new FileOutputStream("times_seq_blur.dat"));
